	...
	return  ['$scope', 'ReportService','$location','$routeParams',
	    function ($scope,reportService,$location,$routeParams) {
            ...
            $scope.createFee = function(){
                var params = $scope.feeParam;
                reportService.createFee(params,function(data){
                    dialog.alert(data.msg);
                });
            };
            $scope.feeParam = {};
            $scope.periodChange = function(){
                var period = $scope.feeParam.period;
            ...
            $scope.selectLocalYear = function(){
                ...
                var months = [];
                if(year == $scope.feeParam.year){
                    if($scope.feeParam.period =='1'){
                        var month = parseInt(new Date().getMonth())+1;
                        for(var i=1;i< month;i++){
                            months.push({code:i+"",text:i+'月'});
                        }
                    }else if($scope.feeParam.period =='2'){
                        var month = parseInt(new Date().getMonth())+1;
                        var n = 0;
                        if(month>0&&month<4){
                            n = 1;
                ...
                }else if(year != $scope.feeParam.year){
                    if($scope.feeParam.period =='1'){
                        for(var i=1;i< 13;i++){
                            months.push({code:i+"",text:i+'月'});
                        }
            ...
            $scope.setRegionName = function(e){
            	console.log(e.target);
            };
            var init = function(){
                $scope.feeParam.period = '1';
                $scope.periodChange();
            };
            init();
	    }
